...
def render_settings_page():
    ...
    try:
        ...
        with col1:
            ...
            st.info(
                f"Current: {current_llm.get('provider', 'unknown')} / {current_llm.get('model', 'unknown')}"
            )
            ...
            new_llm = st.selectbox(
                ...
                options=llm_options,
                index=(
                    llm_options.index(current_llm.get("provider", "ollama"))
                    if current_llm.get("provider") in llm_options
                    else 0
                ),
        ...
        with col2:
            ...
            st.info(
                f"Current: {current_emb.get('provider', 'unknown')} / {current_emb.get('model', 'unknown')}"
            )
            ...
            new_emb = st.selectbox(
                ...
                options=emb_options,
                index=(
                    emb_options.index(current_emb.get("provider", "ollama"))
                    if current_emb.get("provider") in emb_options
                    else 0
                ),
        ...
        if st.button("Apply Provider Changes", key="apply_providers"):
            try:
                result = api.switch_provider(
                    llm_provider=(
                        new_llm if new_llm != current_llm.get("provider") else None
                    ),
                    embedding_provider=(
                        new_emb if new_emb != current_emb.get("provider") else None
                    ),
                )
...
OLLAMA_HOST=http://ollama:11434
    ...
    if st.button("Check Health", key="check_health"):
        try:
            ...
            with col3:
                status = (
                    "✅ Healthy" if health.get("browser_engine") else "❌ Unhealthy"
                )
