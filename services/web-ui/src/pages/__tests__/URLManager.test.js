import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import { QueryClient, QueryClientProvider } from 'react-query';
import { MemoryRouter } from 'react-router-dom';
import '@testing-library/jest-dom';

// Mock the API
jest.mock('../../lib/api', () => ({
  urlAPI: {
    list: jest.fn(),
    getAllUrls: jest.fn(),
    validate: jest.fn(),
    uploadAuto: jest.fn(),
    delete: jest.fn(),
  },
  scrapingAPI: {
    batchScrape: jest.fn(),
  },
  sessionAPI: {
    list: jest.fn(),
    create: jest.fn(),
    get: jest.fn(),
    update: jest.fn(),
    delete: jest.fn(),
    compare: jest.fn(),
    merge: jest.fn(),
    split: jest.fn(),
  },
}));

const createTestQueryClient = () => new QueryClient({
  defaultOptions: {
    queries: {
      retry: false,
      cacheTime: 0,
    },
  },
});

import URLManager from '../URLManager';
import { urlAPI, scrapingAPI, sessionAPI } from '../../lib/api';

const renderWithQueryClient = (ui) => {
  const queryClient = createTestQueryClient();
  return render(
    <QueryClientProvider client={queryClient}>
      <MemoryRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
        {ui}
      </MemoryRouter>
    </QueryClientProvider>
  );
};

const defaultUrlsResponse = {
  data: [
    {
      id: '1',
      url: 'https://example.com',
      domain: 'example.com',
      title: 'Example Site',
      status: 'completed',
      created_at: '2024-01-01T00:00:00Z'
    }
  ]
};

const defaultSessionsResponse = {
  data: [
    {
      id: 'session-1',
      name: 'Test Session',
    },
  ],
};

beforeEach(() => {
  urlAPI.list.mockReset().mockResolvedValue(defaultUrlsResponse);
  urlAPI.getAllUrls.mockReset().mockResolvedValue(defaultUrlsResponse);
  urlAPI.validate.mockReset().mockResolvedValue({ data: { valid: true } });
  urlAPI.uploadAuto.mockReset().mockResolvedValue({ data: { job_id: 'test-job' } });
  urlAPI.delete.mockReset().mockResolvedValue({});
  scrapingAPI.batchScrape.mockReset().mockResolvedValue({ data: { job_id: 'scrape-job' } });
  sessionAPI.list.mockReset().mockResolvedValue(defaultSessionsResponse);
});

test('renders URL manager page', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  expect(await screen.findByText('URL Manager', {}, { timeout: 5000 })).toBeInTheDocument();
  expect(await screen.findByText('Manage your URLs and organize them into collections', {}, { timeout: 5000 })).toBeInTheDocument();
  const urlCells = await screen.findAllByText((content) => content.includes('https://example.com'), {}, { timeout: 5000 });
  expect(urlCells.length).toBeGreaterThan(0);
});

test('shows add URL button', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  expect(
    await screen.findByRole('button', { name: 'Add URL' }, { timeout: 5000 })
  ).toBeInTheDocument();
  expect(await screen.findByText('Upload File', {}, { timeout: 5000 })).toBeInTheDocument();
});

test('search functionality works', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  const searchInput = await screen.findByPlaceholderText('Search URLs...', {}, { timeout: 5000 });
  expect(searchInput).toBeInTheDocument();
  
  fireEvent.change(searchInput, { target: { value: 'example' } });
  expect(searchInput.value).toBe('example');
});

test('status filter works', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  const statusFilter = await screen.findByRole('combobox', { name: /status/i }, { timeout: 5000 });
  expect(statusFilter).toBeInTheDocument();
  
  fireEvent.change(statusFilter, { target: { value: 'completed' } });
  expect(statusFilter.value).toBe('completed');
});

test('URL selection works', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  const checkboxes = await screen.findAllByRole('checkbox', {}, { timeout: 5000 });
  expect(checkboxes.length).toBeGreaterThan(0);
});

test('opens add URL modal', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  const addButton = await screen.findByText('Add URL', {}, { timeout: 5000 });
  expect(addButton).toBeInTheDocument();
  
  fireEvent.click(addButton);
  
  expect(
    await screen.findByRole('heading', { name: 'Add URL' }, { timeout: 5000 })
  ).toBeInTheDocument();
});

test('opens upload modal', async () => {
  renderWithQueryClient(<URLManager />);
  await waitFor(() => expect(sessionAPI.list).toHaveBeenCalled());
  await waitFor(() => expect(urlAPI.getAllUrls).toHaveBeenCalled());
  
  const uploadButton = await screen.findByText('Upload File', {}, { timeout: 5000 });
  expect(uploadButton).toBeInTheDocument();
  
  fireEvent.click(uploadButton);
  
  expect(await screen.findByText('Upload URL File', {}, { timeout: 5000 })).toBeInTheDocument();
});
