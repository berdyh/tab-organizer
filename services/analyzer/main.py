...
class QdrantManager:
    ...
    async def fetch_content_for_session(self, session_id: str) -> List[ContentItem]:
        # This assumes content is already in Qdrant, perhaps stored by scraper or url-input
        # If scraper stores raw content, we can fetch it.
        # For now, let's assume we search for all points
        try:
            ...
            for point in points:
                ...
                # Assuming payload has text and other fields
...
async def process_analysis_job(
    ...
    try:
        ...
        # Determine vector size for cloud embedder if needed
        if cloud_embedder and vector_size == 768:
             try:
                test_emb = await cloud_embedder.get_embedding("test", model=embedding_model_name)
