...
| Frozen security-invariant suite (SECSUITE 1.1.0) | replace-with-contract | Test Harness | Black-box probes SEC-1..39 in `tests/security/` freeze URL safety, CDP local-only, token scopes/fail-closed auth, credential isolation, agent subprocess hardening, prompt envelope, CI secret scanning (`.gitleaks.toml` + `secret-scan` job), and WI0-B6 auth fixtures. FROZEN: edits need a ledger row here + a `SECSUITE_VERSION` bump. Excludes `/platform/*` (decision 41). Challenge fixtures xfail until the B6 fix deletes the marker. **1.1.0**: added the `backend_ingest_v1` door to the token-scope matrix (`POST /api/v1/ingest/v1` unauth/foreign-principal → 401, callback token allowed) alongside the single-ingest-endpoint change. |
